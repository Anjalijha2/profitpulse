        ...
        if (active && payload && payload.length) {
            return (
                <div style={{ background: 'rgba(11, 20, 48, 0.95)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.1)', padding: '12px', borderRadius: '10px', boxShadow: 'var(--shadow-dropdown)' }}>
                    <p style={{ margin: '0 0 8px 0', fontWeight: 600, color: '#FFFFFF' }}>{payload[0].payload.fullName}</p>
                    {payload.map(p => (
                ...
                <Col span={24}>
                    ...
                    >
                        <Table
                            ...
                            rowKey="id"
                            loading={isLoading}
                            pagination={tablePagination(employees.length, currentPage, pageSize, (page, size) => { setCurrentPage(page); setPageSize(size); })}
                        />
                    </Card>
                </Col>
            </Row>
